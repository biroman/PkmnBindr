...
/* Local sets JSON is now generated into public/sets/sets.json at build time. */
let localSetsDataPromise = null;
async function loadLocalSetsData() {
  if (localSetsDataPromise) return localSetsDataPromise;
  const url = `${import.meta.env.BASE_URL || "/"}sets/sets.json`;
  localSetsDataPromise = fetch(url)
    .then((res) => (res.ok ? res.json() : null))
    .catch(() => null);
  return localSetsDataPromise;
}

// Fetch static metadata arrays (types, subtypes, etc.) from public folder
async function loadLocalMeta(name) {
  const url = `${import.meta.env.BASE_URL || "/"}${name}/${name}.json`;
  try {
...
export const pokemonTcgApi = {
    ...
    if (!filters.set) {
      ...
      try {
        ...
        if (!localCardsCache.has(cacheKey)) {
          ...
          const localSets = await loadLocalSetsData();
  ...
  async getTypes() {
    const local = await loadLocalMeta("types");
  ...
  async getSubtypes() {
    const local = await loadLocalMeta("subtypes");
    if (local) return local;
    try {
  ...
  async getRarities() {
    const local = await loadLocalMeta("rarities");
    if (local) return local;
    try {
  ...
  async getSupertypes() {
    const local = await loadLocalMeta("supertypes");
    if (local) return local;
    try {
